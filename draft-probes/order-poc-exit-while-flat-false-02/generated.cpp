...
public:
    ...
    struct _PFScriptState {
        ...
        decltype(GeneratedStrategy::emaFast) _pf_value_4;
        decltype(GeneratedStrategy::emaSlow) _pf_value_5;
        decltype(GeneratedStrategy::sig) _pf_value_6;
        decltype(GeneratedStrategy::exitSig) _pf_value_7;
        decltype(GeneratedStrategy::lvl) _pf_value_8;
    ...
    void restore_script_state() override {
        ...
        this->emaFast = _pf_script_state_checkpoint_->_pf_value_4;
        this->emaSlow = _pf_script_state_checkpoint_->_pf_value_5;
        this->sig = _pf_script_state_checkpoint_->_pf_value_6;
        this->exitSig = _pf_script_state_checkpoint_->_pf_value_7;
        this->lvl = _pf_script_state_checkpoint_->_pf_value_8;
...
#error "Generated lifecycle reset requires a matching PineForge engine; rebuild with script-run preparation support"
    ...
    void on_source_bar(const Bar& bar) override {
        ...
        sig = (history_advances_new_bar() ? _ta_crossover_3.compute(emaFast, emaSlow) : _ta_crossover_3.recompute(emaFast, emaSlow));
        exitSig = (history_advances_new_bar() ? _ta_crossunder_4.compute(emaFast, emaSlow) : _ta_crossunder_4.recompute(emaFast, emaSlow));
        if ((sig && ([&]{ auto _pna_l = (signed_position_size()); auto _pna_r = (0); double _pfc_l = static_cast<double>(_pna_l); double _pfc_r = static_cast<double>(_pna_r); bool _pfc_eq = (_pfc_l == _pfc_r) || (std::isfinite(_pfc_l) && std::isfinite(_pfc_r) && std::fabs(_pfc_l - _pfc_r) <= 1e-10); return !is_na(_pna_l) && !is_na(_pna_r) && (_pfc_eq); }()))) {
            lvl = current_bar_.close;
            strategy_entry(std::string("L"), true, (lvl * 0.98), na<double>(), na<double>(), "", "", 0, -1);
            strategy_exit(std::string("X"), std::string("L"), (lvl * 1.04), (lvl * 0.965), na<double>(), na<double>(), na<double>(), 100.0, "", na<double>(), "", na<double>(), na<double>());
        }
        if ((exitSig && ([&]{ auto _pna_l = (signed_position_size()); auto _pna_r = (0); double _pfc_l = static_cast<double>(_pna_l); double _pfc_r = static_cast<double>(_pna_r); bool _pfc_eq = (_pfc_l == _pfc_r) || (std::isfinite(_pfc_l) && std::isfinite(_pfc_r) && std::fabs(_pfc_l - _pfc_r) <= 1e-10); return !is_na(_pna_l) && !is_na(_pna_r) && ((_pfc_l > _pfc_r) && !_pfc_eq); }()))) {
            strategy_close(std::string("L"), std::string("opp"), na<double>(), na<double>(), false, 184683593747ULL);
        }
    }
