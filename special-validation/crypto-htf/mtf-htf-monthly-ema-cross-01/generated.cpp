...
public:
    ...
    void on_bar(const Bar& bar) override {
        mema = _req_sec_0;
        if ((is_first_tick_ ? _ta_crossover_2.compute(current_bar_.close, mema) : _ta_crossover_2.recompute(current_bar_.close, mema))) {
            strategy_entry(std::string("L"), true, na<double>(), na<double>(), na<double>(), std::string("close x-over monthly EMA"));
        }
        if ((is_first_tick_ ? _ta_crossunder_3.compute(current_bar_.close, mema) : _ta_crossunder_3.recompute(current_bar_.close, mema))) {
