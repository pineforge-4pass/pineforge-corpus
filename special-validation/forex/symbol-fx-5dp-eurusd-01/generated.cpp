...
public:
    ...
    void on_bar(const Bar& bar) override {
        ...
        longCond = (is_first_tick_ ? _ta_crossover_3.compute(fast, slow) : _ta_crossover_3.recompute(fast, slow));
        shortCond = (is_first_tick_ ? _ta_crossunder_4.compute(fast, slow) : _ta_crossunder_4.recompute(fast, slow));
        if (longCond) {
            strategy_close(std::string("S"), std::string("close short"), na<double>(), na<double>(), false);
            strategy_entry(std::string("L"), true, na<double>(), na<double>(), na<double>(), std::string("sma cross up"));
        }
        if (shortCond) {
            strategy_close(std::string("L"), std::string("close long"), na<double>(), na<double>(), false);
