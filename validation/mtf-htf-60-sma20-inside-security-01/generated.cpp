...
public:
    ...
    struct _PFScriptState {
        ...
        decltype(GeneratedStrategy::_ta_sma_1) _pf_value_3;
        decltype(GeneratedStrategy::_sec0__ta_sma_1) _pf_value_4;
        decltype(GeneratedStrategy::htfCl) _pf_value_5;
        decltype(GeneratedStrategy::htfSma) _pf_value_6;
        decltype(GeneratedStrategy::longCond) _pf_value_7;
        decltype(GeneratedStrategy::exitCond) _pf_value_8;
    ...
    void restore_script_state() override {
        ...
        this->_ta_sma_1 = _pf_script_state_checkpoint_->_pf_value_3;
        this->_sec0__ta_sma_1 = _pf_script_state_checkpoint_->_pf_value_4;
        this->htfCl = _pf_script_state_checkpoint_->_pf_value_5;
        this->htfSma = _pf_script_state_checkpoint_->_pf_value_6;
        this->longCond = _pf_script_state_checkpoint_->_pf_value_7;
        this->exitCond = _pf_script_state_checkpoint_->_pf_value_8;
...
#error "Generated lifecycle reset requires a matching PineForge engine; rebuild with script-run preparation support"
    ...
    void prepare_script_run(const Bar* bars, int n, bool allow_precalculation) override {
        ...
        this->_sec0__ta_sma_1 = decltype(this->_sec0__ta_sma_1)(20);
        this->htfCl = decltype(this->htfCl){};
        this->htfSma = decltype(this->htfSma){};
        this->longCond = false;
        this->exitCond = false;
    ...
    void on_source_bar(const Bar& bar) override {
        if (history_advances_new_bar()) htfSma.push(_req_sec_0);
        else htfSma.update(_req_sec_0);
        if (history_advances_new_bar()) htfCl.push(_req_sec_1);
        else htfCl.update(_req_sec_1);
        longCond = (([&]{ auto _pna_l = (htfCl[0]); auto _pna_r = (htfSma[0]); double _pfc_l = static_cast<double>(_pna_l); double _pfc_r = static_cast<double>(_pna_r); bool _pfc_eq = (_pfc_l == _pfc_r) || (std::isfinite(_pfc_l) && std::isfinite(_pfc_r) && std::fabs(_pfc_l - _pfc_r) <= 1e-10); return !is_na(_pna_l) && !is_na(_pna_r) && ((_pfc_l > _pfc_r) && !_pfc_eq); }()) && ([&]{ auto _pna_l = (htfCl[1]); auto _pna_r = (htfSma[1]); double _pfc_l = static_cast<double>(_pna_l); double _pfc_r = static_cast<double>(_pna_r); bool _pfc_eq = (_pfc_l == _pfc_r) || (std::isfinite(_pfc_l) && std::isfinite(_pfc_r) && std::fabs(_pfc_l - _pfc_r) <= 1e-10); return !is_na(_pna_l) && !is_na(_pna_r) && ((_pfc_l < _pfc_r) || _pfc_eq); }()));
        if (longCond) {
            strategy_entry(std::string("L"), true, na<double>(), na<double>(), 1, "", "", 0, -1);
        }
        exitCond = (([&]{ auto _pna_l = (htfCl[0]); auto _pna_r = (htfSma[0]); double _pfc_l = static_cast<double>(_pna_l); double _pfc_r = static_cast<double>(_pna_r); bool _pfc_eq = (_pfc_l == _pfc_r) || (std::isfinite(_pfc_l) && std::isfinite(_pfc_r) && std::fabs(_pfc_l - _pfc_r) <= 1e-10); return !is_na(_pna_l) && !is_na(_pna_r) && ((_pfc_l < _pfc_r) && !_pfc_eq); }()) && ([&]{ auto _pna_l = (signed_position_size()); auto _pna_r = (0); double _pfc_l = static_cast<double>(_pna_l); double _pfc_r = static_cast<double>(_pna_r); bool _pfc_eq = (_pfc_l == _pfc_r) || (std::isfinite(_pfc_l) && std::isfinite(_pfc_r) && std::fabs(_pfc_l - _pfc_r) <= 1e-10); return !is_na(_pna_l) && !is_na(_pna_r) && ((_pfc_l > _pfc_r) && !_pfc_eq); }()));
        if (exitCond) {
            strategy_close(std::string("L"), "", na<double>(), na<double>(), false, 115964117011ULL);
        }
    }
    ...
    void _eval_security_0(const Bar& bar, bool is_complete) {
        auto _secval_0 = security_series_slot_is_new(0) ? _sec0__ta_sma_1.compute(bar.close) : _sec0__ta_sma_1.recompute(bar.close);
