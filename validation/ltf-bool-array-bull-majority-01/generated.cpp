...
#error "Generated lifecycle reset requires a matching PineForge engine; rebuild with script-run preparation support"
    ...
    void on_source_bar(const Bar& bar) override {
        ...
        if (([&]{ auto _pna_l = ((double)ltfBullArr.size()); auto _pna_r = (0); double _pfc_l = static_cast<double>(_pna_l); double _pfc_r = static_cast<double>(_pna_r); bool _pfc_eq = (_pfc_l == _pfc_r) || (std::isfinite(_pfc_l) && std::isfinite(_pfc_r) && std::fabs(_pfc_l - _pfc_r) <= 1e-10); return !is_na(_pna_l) && !is_na(_pna_r) && ((_pfc_l > _pfc_r) && !_pfc_eq); }())) {
            int _for_start_0 = (0);
            int _for_end_0 = (((double)ltfBullArr.size() - 1));
            int _for_step_0 = (1);
            if (_for_step_0 < 0) _for_step_0 = -_for_step_0;
            if (_for_step_0 == 0) _for_step_0 = 1;
            const bool _for_down_0 = (_for_start_0 > _for_end_0);
            for (int i = _for_start_0; (_for_down_0 ? (i >= _for_end_0) : (i <= _for_end_0)); i += (_for_down_0 ? -_for_step_0 : _for_step_0), _for_end_0 = (((double)ltfBullArr.size() - 1))) {
