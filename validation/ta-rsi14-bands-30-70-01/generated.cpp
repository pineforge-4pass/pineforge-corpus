...
#error "Generated lifecycle reset requires a matching PineForge engine; rebuild with script-run preparation support"
    ...
    void on_source_bar(const Bar& bar) override {
        ...
        if ((history_advances_new_bar() ? _ta_crossover_2.compute(rsi, 30.0) : _ta_crossover_2.recompute(rsi, 30.0))) {
            strategy_entry(std::string("L"), true, na<double>(), na<double>(), na<double>(), std::string("rsi cross up 30"));
        }
        if ((history_advances_new_bar() ? _ta_crossunder_3.compute(rsi, 70.0) : _ta_crossunder_3.recompute(rsi, 70.0))) {
