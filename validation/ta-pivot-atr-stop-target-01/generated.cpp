...
public:
    ...
    struct _PFScriptState {
        ...
        decltype(GeneratedStrategy::lastPvtH) _pf_value_4;
        decltype(GeneratedStrategy::lastPvtL) _pf_value_5;
        decltype(GeneratedStrategy::pivotLen) _pf_value_6;
        decltype(GeneratedStrategy::atrLen) _pf_value_7;
        decltype(GeneratedStrategy::atrMult) _pf_value_8;
        decltype(GeneratedStrategy::tpMult) _pf_value_9;
        decltype(GeneratedStrategy::pvtHigh) _pf_value_10;
        decltype(GeneratedStrategy::pvtLow) _pf_value_11;
        decltype(GeneratedStrategy::atrVal) _pf_value_12;
    ...
    void restore_script_state() override {
        ...
        this->lastPvtH = _pf_script_state_checkpoint_->_pf_value_4;
        this->lastPvtL = _pf_script_state_checkpoint_->_pf_value_5;
        this->pivotLen = _pf_script_state_checkpoint_->_pf_value_6;
        this->atrLen = _pf_script_state_checkpoint_->_pf_value_7;
        this->atrMult = _pf_script_state_checkpoint_->_pf_value_8;
        this->tpMult = _pf_script_state_checkpoint_->_pf_value_9;
        this->pvtHigh = _pf_script_state_checkpoint_->_pf_value_10;
        this->pvtLow = _pf_script_state_checkpoint_->_pf_value_11;
        this->atrVal = _pf_script_state_checkpoint_->_pf_value_12;
...
#error "Generated lifecycle reset requires a matching PineForge engine; rebuild with script-run preparation support"
    ...
    void prepare_script_run(const Bar* bars, int n, bool allow_precalculation) override {
        ...
        this->lastPvtH = decltype(this->lastPvtH)(na<double>());
        this->lastPvtL = decltype(this->lastPvtL)(na<double>());
        this->pivotLen = 0;
        this->atrLen = 0;
        this->atrMult = 0.0;
        this->tpMult = 0.0;
    ...
    void on_source_bar(const Bar& bar) override {
        ...
        longCond = ((!(is_na(lastPvtH)) && ([&]{ auto _pna_l = (current_bar_.close); auto _pna_r = (lastPvtH); double _pfc_l = static_cast<double>(_pna_l); double _pfc_r = static_cast<double>(_pna_r); bool _pfc_eq = (_pfc_l == _pfc_r) || (std::isfinite(_pfc_l) && std::isfinite(_pfc_r) && std::fabs(_pfc_l - _pfc_r) <= 1e-10); return !is_na(_pna_l) && !is_na(_pna_r) && ((_pfc_l > _pfc_r) && !_pfc_eq); }())) && ([&]{ auto _pna_l = (_s_close[1]); auto _pna_r = (lastPvtH); double _pfc_l = static_cast<double>(_pna_l); double _pfc_r = static_cast<double>(_pna_r); bool _pfc_eq = (_pfc_l == _pfc_r) || (std::isfinite(_pfc_l) && std::isfinite(_pfc_r) && std::fabs(_pfc_l - _pfc_r) <= 1e-10); return !is_na(_pna_l) && !is_na(_pna_r) && ((_pfc_l < _pfc_r) || _pfc_eq); }()));
        shortCond = ((!(is_na(lastPvtL)) && ([&]{ auto _pna_l = (current_bar_.close); auto _pna_r = (lastPvtL); double _pfc_l = static_cast<double>(_pna_l); double _pfc_r = static_cast<double>(_pna_r); bool _pfc_eq = (_pfc_l == _pfc_r) || (std::isfinite(_pfc_l) && std::isfinite(_pfc_r) && std::fabs(_pfc_l - _pfc_r) <= 1e-10); return !is_na(_pna_l) && !is_na(_pna_r) && ((_pfc_l < _pfc_r) && !_pfc_eq); }())) && ([&]{ auto _pna_l = (_s_close[1]); auto _pna_r = (lastPvtL); double _pfc_l = static_cast<double>(_pna_l); double _pfc_r = static_cast<double>(_pna_r); bool _pfc_eq = (_pfc_l == _pfc_r) || (std::isfinite(_pfc_l) && std::isfinite(_pfc_r) && std::fabs(_pfc_l - _pfc_r) <= 1e-10); return !is_na(_pna_l) && !is_na(_pna_r) && ((_pfc_l > _pfc_r) || _pfc_eq); }()));
        if (longCond) {
            strategy_entry(std::string("Long"), true, na<double>(), na<double>(), na<double>(), "");
            strategy_exit(std::string("XL"), std::string("Long"), (current_bar_.close + (atrVal * tpMult)), (current_bar_.close - (atrVal * atrMult)), na<double>(), na<double>(), na<double>(), 100.0, "", na<double>(), "", na<double>(), na<double>());
        }
        if (shortCond) {
            strategy_entry(std::string("Short"), false, na<double>(), na<double>(), na<double>(), "");
            strategy_exit(std::string("XS"), std::string("Short"), (current_bar_.close - (atrVal * tpMult)), (current_bar_.close + (atrVal * atrMult)), na<double>(), na<double>(), na<double>(), 100.0, "", na<double>(), "", na<double>(), na<double>());
        }
    }
