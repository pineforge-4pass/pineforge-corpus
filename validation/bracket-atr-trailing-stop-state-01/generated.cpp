...
public:
    ...
    struct _PFScriptState {
        ...
        decltype(GeneratedStrategy::trailStop) _pf_value_4;
        decltype(GeneratedStrategy::isLong) _pf_value_5;
        decltype(GeneratedStrategy::atrLen) _pf_value_6;
        decltype(GeneratedStrategy::atrMult) _pf_value_7;
        decltype(GeneratedStrategy::maLen) _pf_value_8;
        decltype(GeneratedStrategy::atrVal) _pf_value_9;
        decltype(GeneratedStrategy::maVal) _pf_value_10;
        decltype(GeneratedStrategy::longEntry) _pf_value_11;
        decltype(GeneratedStrategy::shortEntry) _pf_value_12;
        decltype(GeneratedStrategy::newStop) _pf_value_13;
    ...
    void restore_script_state() override {
        ...
        this->trailStop = _pf_script_state_checkpoint_->_pf_value_4;
        this->isLong = _pf_script_state_checkpoint_->_pf_value_5;
        this->atrLen = _pf_script_state_checkpoint_->_pf_value_6;
        this->atrMult = _pf_script_state_checkpoint_->_pf_value_7;
        this->maLen = _pf_script_state_checkpoint_->_pf_value_8;
        this->atrVal = _pf_script_state_checkpoint_->_pf_value_9;
        this->maVal = _pf_script_state_checkpoint_->_pf_value_10;
        this->longEntry = _pf_script_state_checkpoint_->_pf_value_11;
        this->shortEntry = _pf_script_state_checkpoint_->_pf_value_12;
        this->newStop = _pf_script_state_checkpoint_->_pf_value_13;
...
#error "Generated lifecycle reset requires a matching PineForge engine; rebuild with script-run preparation support"
    ...
    void prepare_script_run(const Bar* bars, int n, bool allow_precalculation) override {
        ...
        this->isLong = decltype(this->isLong)(false);
        this->atrLen = 0;
        this->atrMult = 0.0;
        this->maLen = 0;
        this->atrVal = 0.0;
        this->maVal = 0.0;
        this->longEntry = false;
        this->shortEntry = false;
        this->newStop = 0.0;
    ...
    void on_source_bar(const Bar& bar) override {
        ...
        if ((isLong && ([&]{ auto _pna_l = (signed_position_size()); auto _pna_r = (0); double _pfc_l = static_cast<double>(_pna_l); double _pfc_r = static_cast<double>(_pna_r); bool _pfc_eq = (_pfc_l == _pfc_r) || (std::isfinite(_pfc_l) && std::isfinite(_pfc_r) && std::fabs(_pfc_l - _pfc_r) <= 1e-10); return !is_na(_pna_l) && !is_na(_pna_r) && ((_pfc_l > _pfc_r) && !_pfc_eq); }()))) {
            newStop = (current_bar_.close - (atrVal * atrMult));
            if (!(is_na(trailStop))) {
                trailStop = ([&]() -> double { double _v0 = (double)(trailStop); double _v1 = (double)(newStop); if (is_na(_v0) || is_na(_v1)) return na<double>(); double _out = _v0; _out = std::max(_out, _v1); return _out; }());
            } else {
                trailStop = newStop;
            }
            if (([&]{ auto _pna_l = (current_bar_.close); auto _pna_r = (trailStop); double _pfc_l = static_cast<double>(_pna_l); double _pfc_r = static_cast<double>(_pna_r); bool _pfc_eq = (_pfc_l == _pfc_r) || (std::isfinite(_pfc_l) && std::isfinite(_pfc_r) && std::fabs(_pfc_l - _pfc_r) <= 1e-10); return !is_na(_pna_l) && !is_na(_pna_r) && ((_pfc_l < _pfc_r) && !_pfc_eq); }())) {
                strategy_close(std::string("Long"), "", na<double>(), na<double>(), false, 201863462935ULL);
                isLong = false;
            }
        }
        if ((!(isLong) && ([&]{ auto _pna_l = (signed_position_size()); auto _pna_r = (0); double _pfc_l = static_cast<double>(_pna_l); double _pfc_r = static_cast<double>(_pna_r); bool _pfc_eq = (_pfc_l == _pfc_r) || (std::isfinite(_pfc_l) && std::isfinite(_pfc_r) && std::fabs(_pfc_l - _pfc_r) <= 1e-10); return !is_na(_pna_l) && !is_na(_pna_r) && ((_pfc_l < _pfc_r) && !_pfc_eq); }()))) {
            newStop = (current_bar_.close + (atrVal * atrMult));
            if (!(is_na(trailStop))) {
                trailStop = ([&]() -> double { double _v0 = (double)(trailStop); double _v1 = (double)(newStop); if (is_na(_v0) || is_na(_v1)) return na<double>(); double _out = _v0; _out = std::min(_out, _v1); return _out; }());
            } else {
                trailStop = newStop;
            }
            if (([&]{ auto _pna_l = (current_bar_.close); auto _pna_r = (trailStop); double _pfc_l = static_cast<double>(_pna_l); double _pfc_r = static_cast<double>(_pna_r); bool _pfc_eq = (_pfc_l == _pfc_r) || (std::isfinite(_pfc_l) && std::isfinite(_pfc_r) && std::fabs(_pfc_l - _pfc_r) <= 1e-10); return !is_na(_pna_l) && !is_na(_pna_r) && ((_pfc_l > _pfc_r) && !_pfc_eq); }())) {
                strategy_close(std::string("Short"), "", na<double>(), na<double>(), false, 244813135895ULL);
            }
        }
    }
