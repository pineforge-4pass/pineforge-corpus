...
#error "Generated lifecycle reset requires a matching PineForge engine; rebuild with script-run preparation support"
    ...
    void on_source_bar(const Bar& bar) override {
        ...
        for (int offset = _for_start_0; (_for_down_0 ? (offset >= _for_end_0) : (offset <= _for_end_0)); offset += (_for_down_0 ? -_for_step_0 : _for_step_0), _for_end_0 = (channelLength)) {
            channelHigh = ([&]() -> double { double _v0 = (double)(channelHigh); double _v1 = (double)(_s_high[([&](){ auto _pf_idx_v = (offset); using _pf_idx_t = std::decay_t<decltype(_pf_idx_v)>; if constexpr (std::is_same_v<_pf_idx_t, bool>) return (int)_pf_idx_v; else return is_na(_pf_idx_v) ? na<int>() : (int)_pf_idx_v; }())]); if (is_na(_v0) || is_na(_v1)) return na<double>(); double _out = _v0; _out = std::max(_out, _v1); return _out; }());
            channelLow = ([&]() -> double { double _v0 = (double)(channelLow); double _v1 = (double)(_s_low[([&](){ auto _pf_idx_v = (offset); using _pf_idx_t = std::decay_t<decltype(_pf_idx_v)>; if constexpr (std::is_same_v<_pf_idx_t, bool>) return (int)_pf_idx_v; else return is_na(_pf_idx_v) ? na<int>() : (int)_pf_idx_v; }())]); if (is_na(_v0) || is_na(_v1)) return na<double>(); double _out = _v0; _out = std::min(_out, _v1); return _out; }());
            logMovement += std::abs(std::log(((double)(_s_close[([&](){ auto _pf_idx_v = ((offset - 1)); using _pf_idx_t = std::decay_t<decltype(_pf_idx_v)>; if constexpr (std::is_same_v<_pf_idx_t, bool>) return (int)_pf_idx_v; else return is_na(_pf_idx_v) ? na<int>() : (int)_pf_idx_v; }())]) / (double)(_s_close[([&](){ auto _pf_idx_v = (offset); using _pf_idx_t = std::decay_t<decltype(_pf_idx_v)>; if constexpr (std::is_same_v<_pf_idx_t, bool>) return (int)_pf_idx_v; else return is_na(_pf_idx_v) ? na<int>() : (int)_pf_idx_v; }())]))));
        }
