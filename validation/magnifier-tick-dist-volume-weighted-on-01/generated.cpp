...
public:
    ...
    struct _PFScriptState {
        ...
        decltype(GeneratedStrategy::entryCond) _pf_value_6;
        decltype(GeneratedStrategy::exitCond) _pf_value_7;
        decltype(GeneratedStrategy::stopLvl) _pf_value_8;
    ...
    void restore_script_state() override {
        ...
        this->entryCond = _pf_script_state_checkpoint_->_pf_value_6;
        this->exitCond = _pf_script_state_checkpoint_->_pf_value_7;
        this->stopLvl = _pf_script_state_checkpoint_->_pf_value_8;
...
#error "Generated lifecycle reset requires a matching PineForge engine; rebuild with script-run preparation support"
    ...
    void on_source_bar(const Bar& bar) override {
        ...
        entryCond = (history_advances_new_bar() ? _ta_crossover_3.compute(emaFast, emaSlow) : _ta_crossover_3.recompute(emaFast, emaSlow));
        ...
        if ((exitCond && ([&]{ auto _pna_l = (signed_position_size()); auto _pna_r = (0); double _pfc_l = static_cast<double>(_pna_l); double _pfc_r = static_cast<double>(_pna_r); bool _pfc_eq = (_pfc_l == _pfc_r) || (std::isfinite(_pfc_l) && std::isfinite(_pfc_r) && std::fabs(_pfc_l - _pfc_r) <= 1e-10); return !is_na(_pna_l) && !is_na(_pna_r) && ((_pfc_l > _pfc_r) && !_pfc_eq); }()))) {
            strategy_close(std::string("L"), std::string("exit long"), na<double>(), na<double>(), false, 176093659155ULL);
        }
    }
