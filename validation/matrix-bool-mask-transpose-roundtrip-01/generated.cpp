...
public:
    ...
    struct _PFScriptState {
        ...
        _PFCheckpointTraits<decltype(GeneratedStrategy::entryCond)>::snapshot_type _pf_value_8;
        _PFCheckpointTraits<decltype(GeneratedStrategy::exitCond)>::snapshot_type _pf_value_9;
    ...
    void restore_script_state() override {
        ...
        _PFCheckpointTraits<decltype(GeneratedStrategy::entryCond)>::restore(this->entryCond, _pf_script_state_checkpoint_->_pf_value_8);
        _PFCheckpointTraits<decltype(GeneratedStrategy::exitCond)>::restore(this->exitCond, _pf_script_state_checkpoint_->_pf_value_9);
...
#error "Generated lifecycle reset requires a matching PineForge engine; rebuild with script-run preparation support"
    ...
    void on_source_bar(const Bar& bar) override {
        ...
        entryCond = (sample && ([&]{ auto _pna_l = (rsiVal); auto _pna_r = (55.0); double _pfc_l = static_cast<double>(_pna_l); double _pfc_r = static_cast<double>(_pna_r); bool _pfc_eq = (_pfc_l == _pfc_r) || (std::isfinite(_pfc_l) && std::isfinite(_pfc_r) && std::fabs(_pfc_l - _pfc_r) <= 1e-10); return !is_na(_pna_l) && !is_na(_pna_r) && ((_pfc_l > _pfc_r) && !_pfc_eq); }()));
        ...
        if ((exitCond && ([&]{ auto _pna_l = (signed_position_size()); auto _pna_r = (0); double _pfc_l = static_cast<double>(_pna_l); double _pfc_r = static_cast<double>(_pna_r); bool _pfc_eq = (_pfc_l == _pfc_r) || (std::isfinite(_pfc_l) && std::isfinite(_pfc_r) && std::fabs(_pfc_l - _pfc_r) <= 1e-10); return !is_na(_pna_l) && !is_na(_pna_r) && ((_pfc_l > _pfc_r) && !_pfc_eq); }()))) {
            strategy_close(std::string("L"), std::string("exit long"), na<double>(), na<double>(), false, 163208757267ULL);
        }
    }
