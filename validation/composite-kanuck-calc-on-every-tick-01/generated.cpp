...
public:
    ...
    struct _PFScriptState {
        ...
        decltype(GeneratedStrategy::tick_counter) _pf_value_4;
        decltype(GeneratedStrategy::i_step) _pf_value_5;
        decltype(GeneratedStrategy::i_fast) _pf_value_6;
        ...
        decltype(GeneratedStrategy::cross_up) _pf_value_10;
        decltype(GeneratedStrategy::cross_down) _pf_value_11;
        decltype(GeneratedStrategy::gate) _pf_value_12;
    ...
    void restore_script_state() override {
        ...
        this->tick_counter = _pf_script_state_checkpoint_->_pf_value_4;
        this->i_step = _pf_script_state_checkpoint_->_pf_value_5;
        this->i_fast = _pf_script_state_checkpoint_->_pf_value_6;
        ...
        this->cross_up = _pf_script_state_checkpoint_->_pf_value_10;
        this->cross_down = _pf_script_state_checkpoint_->_pf_value_11;
        this->gate = _pf_script_state_checkpoint_->_pf_value_12;
...
#error "Generated lifecycle reset requires a matching PineForge engine; rebuild with script-run preparation support"
    ...
    void on_source_bar(const Bar& bar) override {
        ...
        cross_up = (history_advances_new_bar() ? _ta_crossover_3.compute(fast, slow) : _ta_crossover_3.recompute(fast, slow));
        cross_down = (history_advances_new_bar() ? _ta_crossunder_4.compute(fast, slow) : _ta_crossunder_4.recompute(fast, slow));
        ...
        if (((cross_up && gate) && ([&]{ auto _pna_l = (signed_position_size()); auto _pna_r = (0); double _pfc_l = static_cast<double>(_pna_l); double _pfc_r = static_cast<double>(_pna_r); bool _pfc_eq = (_pfc_l == _pfc_r) || (std::isfinite(_pfc_l) && std::isfinite(_pfc_r) && std::fabs(_pfc_l - _pfc_r) <= 1e-10); return !is_na(_pna_l) && !is_na(_pna_r) && ((_pfc_l < _pfc_r) || _pfc_eq); }()))) {
            strategy_entry(std::string("L"), true, na<double>(), na<double>(), 1, std::string("tick gated long"), "", 0, -1);
        }
        if (((cross_down && gate) && ([&]{ auto _pna_l = (signed_position_size()); auto _pna_r = (0); double _pfc_l = static_cast<double>(_pna_l); double _pfc_r = static_cast<double>(_pna_r); bool _pfc_eq = (_pfc_l == _pfc_r) || (std::isfinite(_pfc_l) && std::isfinite(_pfc_r) && std::fabs(_pfc_l - _pfc_r) <= 1e-10); return !is_na(_pna_l) && !is_na(_pna_r) && ((_pfc_l > _pfc_r) || _pfc_eq); }()))) {
