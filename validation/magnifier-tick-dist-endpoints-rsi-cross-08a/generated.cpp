...
public:
    ...
    struct _PFScriptState {
        ...
        decltype(GeneratedStrategy::entryCond) _pf_value_4;
        decltype(GeneratedStrategy::exitCond) _pf_value_5;
        decltype(GeneratedStrategy::stopLvl) _pf_value_6;
    ...
    void restore_script_state() override {
        ...
        this->entryCond = _pf_script_state_checkpoint_->_pf_value_4;
        this->exitCond = _pf_script_state_checkpoint_->_pf_value_5;
        this->stopLvl = _pf_script_state_checkpoint_->_pf_value_6;
...
#error "Generated lifecycle reset requires a matching PineForge engine; rebuild with script-run preparation support"
    ...
    void on_source_bar(const Bar& bar) override {
        ...
        entryCond = (history_advances_new_bar() ? _ta_crossover_2.compute(rsiVal, 50.0) : _ta_crossover_2.recompute(rsiVal, 50.0));
        exitCond = (history_advances_new_bar() ? _ta_crossunder_3.compute(rsiVal, 50.0) : _ta_crossunder_3.recompute(rsiVal, 50.0));
        if ((entryCond && ([&]{ auto _pna_l = (signed_position_size()); auto _pna_r = (0); double _pfc_l = static_cast<double>(_pna_l); double _pfc_r = static_cast<double>(_pna_r); bool _pfc_eq = (_pfc_l == _pfc_r) || (std::isfinite(_pfc_l) && std::isfinite(_pfc_r) && std::fabs(_pfc_l - _pfc_r) <= 1e-10); return !is_na(_pna_l) && !is_na(_pna_r) && (_pfc_eq); }()))) {
            ...
            strategy_exit(std::string("X"), std::string("L"), na<double>(), stopLvl, na<double>(), na<double>(), na<double>(), 100.0, std::string("mid-bar stop"), na<double>(), "", na<double>(), na<double>());
        }
        if ((exitCond && ([&]{ auto _pna_l = (signed_position_size()); auto _pna_r = (0); double _pfc_l = static_cast<double>(_pna_l); double _pfc_r = static_cast<double>(_pna_r); bool _pfc_eq = (_pfc_l == _pfc_r) || (std::isfinite(_pfc_l) && std::isfinite(_pfc_r) && std::fabs(_pfc_l - _pfc_r) <= 1e-10); return !is_na(_pna_l) && !is_na(_pna_r) && ((_pfc_l > _pfc_r) && !_pfc_eq); }()))) {
            strategy_close(std::string("L"), std::string("exit long"), na<double>(), na<double>(), false, 158913789971ULL);
        }
    }
