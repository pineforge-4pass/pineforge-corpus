...
public:
    ...
    struct _PFScriptState {
        ...
        decltype(GeneratedStrategy::_sec0_hist_high) _pf_value_1;
        ...
        decltype(GeneratedStrategy::dHigh1) _pf_value_5;
        decltype(GeneratedStrategy::crossUp) _pf_value_6;
    ...
    void restore_script_state() override {
        ...
        this->_sec0_hist_high = _pf_script_state_checkpoint_->_pf_value_1;
        ...
        this->dHigh1 = _pf_script_state_checkpoint_->_pf_value_5;
        this->crossUp = _pf_script_state_checkpoint_->_pf_value_6;
...
#error "Generated lifecycle reset requires a matching PineForge engine; rebuild with script-run preparation support"
    ...
    void prepare_script_run(const Bar* bars, int n, bool allow_precalculation) override {
        ...
        this->dHigh1 = decltype(this->dHigh1){};
        this->crossUp = false;
    ...
    void on_source_bar(const Bar& bar) override {
        ...
        if (history_advances_new_bar()) dHigh1.push(_req_sec_0);
        else dHigh1.update(_req_sec_0);
        ...
        if ((([&]{ auto _pna_l = (current_bar_.close); auto _pna_r = ((dHigh1[0] * 0.995)); double _pfc_l = static_cast<double>(_pna_l); double _pfc_r = static_cast<double>(_pna_r); bool _pfc_eq = (_pfc_l == _pfc_r) || (std::isfinite(_pfc_l) && std::isfinite(_pfc_r) && std::fabs(_pfc_l - _pfc_r) <= 1e-10); return !is_na(_pna_l) && !is_na(_pna_r) && ((_pfc_l < _pfc_r) && !_pfc_eq); }()) && ([&]{ auto _pna_l = (signed_position_size()); auto _pna_r = (0); double _pfc_l = static_cast<double>(_pna_l); double _pfc_r = static_cast<double>(_pna_r); bool _pfc_eq = (_pfc_l == _pfc_r) || (std::isfinite(_pfc_l) && std::isfinite(_pfc_r) && std::fabs(_pfc_l - _pfc_r) <= 1e-10); return !is_na(_pna_l) && !is_na(_pna_r) && ((_pfc_l > _pfc_r) && !_pfc_eq); }()))) {
            strategy_close(std::string("L"), "", na<double>(), na<double>(), false, 137438953491ULL);
        }
    }
