...
public:
    ...
    struct _PFScriptState {
        ...
        decltype(GeneratedStrategy::pendingLong) _pf_value_4;
        decltype(GeneratedStrategy::pendingExit) _pf_value_5;
    ...
    void restore_script_state() override {
        ...
        this->pendingLong = _pf_script_state_checkpoint_->_pf_value_4;
        this->pendingExit = _pf_script_state_checkpoint_->_pf_value_5;
...
#error "Generated lifecycle reset requires a matching PineForge engine; rebuild with script-run preparation support"
    ...
    void prepare_script_run(const Bar* bars, int n, bool allow_precalculation) override {
        ...
        this->pendingLong = decltype(this->pendingLong)(false);
        this->pendingExit = decltype(this->pendingExit)(false);
        this->emaFast = 0.0;
        this->emaSlow = 0.0;
    ...
    void on_source_bar(const Bar& bar) override {
        ...
        if ((history_advances_new_bar() ? _ta_crossover_3.compute(emaFast, emaSlow) : _ta_crossover_3.recompute(emaFast, emaSlow))) {
            pendingLong = true;
        }
        if ((history_advances_new_bar() ? _ta_crossunder_4.compute(emaFast, emaSlow) : _ta_crossunder_4.recompute(emaFast, emaSlow))) {
            pendingExit = true;
        }
        if (((is_last_tick_ && pendingLong) && ([&]{ auto _pna_l = (signed_position_size()); auto _pna_r = (0); double _pfc_l = static_cast<double>(_pna_l); double _pfc_r = static_cast<double>(_pna_r); bool _pfc_eq = (_pfc_l == _pfc_r) || (std::isfinite(_pfc_l) && std::isfinite(_pfc_r) && std::fabs(_pfc_l - _pfc_r) <= 1e-10); return !is_na(_pna_l) && !is_na(_pna_r) && (_pfc_eq); }()))) {
            strategy_entry(std::string("L"), true, na<double>(), na<double>(), 1, std::string("confirmed long"), "", 0, -1);
            pendingLong = false;
        }
        if (((is_last_tick_ && pendingExit) && ([&]{ auto _pna_l = (signed_position_size()); auto _pna_r = (0); double _pfc_l = static_cast<double>(_pna_l); double _pfc_r = static_cast<double>(_pna_r); bool _pfc_eq = (_pfc_l == _pfc_r) || (std::isfinite(_pfc_l) && std::isfinite(_pfc_r) && std::fabs(_pfc_l - _pfc_r) <= 1e-10); return !is_na(_pna_l) && !is_na(_pna_r) && ((_pfc_l > _pfc_r) && !_pfc_eq); }()))) {
            strategy_close(std::string("L"), std::string("confirmed exit"), na<double>(), na<double>(), false, 184683593747ULL);
            pendingExit = false;
        }
    }
