...
public:
    ...
    struct _PFScriptState {
        ...
        decltype(GeneratedStrategy::roll) _pf_value_3;
    ...
    void restore_script_state() override {
        ...
        this->roll = _pf_script_state_checkpoint_->_pf_value_3;
...
#error "Generated lifecycle reset requires a matching PineForge engine; rebuild with script-run preparation support"
    ...
    void on_source_bar(const Bar& bar) override {
        h60 = _req_sec_0;
        if (history_advances_new_bar()) roll.push(([&]{ auto _pna_l = ((history_advances_new_bar() ? _ta_change_1.compute(h60) : _ta_change_1.recompute(h60))); auto _pna_r = (0); double _pfc_l = static_cast<double>(_pna_l); double _pfc_r = static_cast<double>(_pna_r); bool _pfc_eq = (_pfc_l == _pfc_r) || (std::isfinite(_pfc_l) && std::isfinite(_pfc_r) && std::fabs(_pfc_l - _pfc_r) <= 1e-10); return !is_na(_pna_l) && !is_na(_pna_r) && (!_pfc_eq); }()));
        else roll.update(([&]{ auto _pna_l = ((history_advances_new_bar() ? _ta_change_1.compute(h60) : _ta_change_1.recompute(h60))); auto _pna_r = (0); double _pfc_l = static_cast<double>(_pna_l); double _pfc_r = static_cast<double>(_pna_r); bool _pfc_eq = (_pfc_l == _pfc_r) || (std::isfinite(_pfc_l) && std::isfinite(_pfc_r) && std::fabs(_pfc_l - _pfc_r) <= 1e-10); return !is_na(_pna_l) && !is_na(_pna_r) && (!_pfc_eq); }()));
        if (roll[0]) {
            strategy_entry(std::string("R"), true, na<double>(), na<double>(), 1, "", "", 0, -1);
        }
        if (roll[1]) {
            strategy_close(std::string("R"), "", na<double>(), na<double>(), false, 94489280531ULL);
        }
        if ((barstate_islast_ && ([&]{ auto _pna_l = (signed_position_size()); auto _pna_r = (0); double _pfc_l = static_cast<double>(_pna_l); double _pfc_r = static_cast<double>(_pna_r); bool _pfc_eq = (_pfc_l == _pfc_r) || (std::isfinite(_pfc_l) && std::isfinite(_pfc_r) && std::fabs(_pfc_l - _pfc_r) <= 1e-10); return !is_na(_pna_l) && !is_na(_pna_r) && ((_pfc_l > _pfc_r) && !_pfc_eq); }()))) {
            strategy_close(std::string("R"), "", na<double>(), na<double>(), false, 107374182419ULL);
        }
    }
