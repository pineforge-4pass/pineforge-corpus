...
public:
    ...
    struct _PFScriptState {
        ...
        decltype(GeneratedStrategy::src) _pf_value_4;
        decltype(GeneratedStrategy::fastLen) _pf_value_5;
        decltype(GeneratedStrategy::slowLen) _pf_value_6;
        decltype(GeneratedStrategy::fast) _pf_value_7;
        decltype(GeneratedStrategy::slow) _pf_value_8;
    ...
    void restore_script_state() override {
        ...
        this->src = _pf_script_state_checkpoint_->_pf_value_4;
        this->fastLen = _pf_script_state_checkpoint_->_pf_value_5;
        this->slowLen = _pf_script_state_checkpoint_->_pf_value_6;
        this->fast = _pf_script_state_checkpoint_->_pf_value_7;
        this->slow = _pf_script_state_checkpoint_->_pf_value_8;
...
#error "Generated lifecycle reset requires a matching PineForge engine; rebuild with script-run preparation support"
    ...
    void prepare_script_run(const Bar* bars, int n, bool allow_precalculation) override {
        ...
        this->_ta_sma_2 = decltype(this->_ta_sma_2)(30);
    ...
    void on_source_bar(const Bar& bar) override {
        ...
        fast = (history_advances_new_bar() ? _ta_sma_1.compute(src) : _ta_sma_1.recompute(src));
        slow = (history_advances_new_bar() ? _ta_sma_2.compute(src) : _ta_sma_2.recompute(src));
        if ((history_advances_new_bar() ? _ta_crossover_3.compute(fast, slow) : _ta_crossover_3.recompute(fast, slow))) {
            strategy_entry(std::string("Long"), true, na<double>(), na<double>(), na<double>(), "");
        }
        if ((history_advances_new_bar() ? _ta_crossunder_4.compute(fast, slow) : _ta_crossunder_4.recompute(fast, slow))) {
