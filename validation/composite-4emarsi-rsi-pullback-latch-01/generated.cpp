...
public:
    ...
    struct _PFScriptState {
        ...
        decltype(GeneratedStrategy::long_armed) _pf_value_3;
        decltype(GeneratedStrategy::short_armed) _pf_value_4;
        decltype(GeneratedStrategy::i_rsi_len) _pf_value_5;
        decltype(GeneratedStrategy::i_dip_lo) _pf_value_6;
        decltype(GeneratedStrategy::i_pop_lo) _pf_value_7;
        decltype(GeneratedStrategy::i_dip_hi) _pf_value_8;
        decltype(GeneratedStrategy::i_pop_hi) _pf_value_9;
        decltype(GeneratedStrategy::r) _pf_value_10;
        decltype(GeneratedStrategy::long_fire) _pf_value_11;
        decltype(GeneratedStrategy::short_fire) _pf_value_12;
    ...
    void restore_script_state() override {
        ...
        this->long_armed = _pf_script_state_checkpoint_->_pf_value_3;
        this->short_armed = _pf_script_state_checkpoint_->_pf_value_4;
        this->i_rsi_len = _pf_script_state_checkpoint_->_pf_value_5;
        this->i_dip_lo = _pf_script_state_checkpoint_->_pf_value_6;
        this->i_pop_lo = _pf_script_state_checkpoint_->_pf_value_7;
        this->i_dip_hi = _pf_script_state_checkpoint_->_pf_value_8;
        this->i_pop_hi = _pf_script_state_checkpoint_->_pf_value_9;
        this->r = _pf_script_state_checkpoint_->_pf_value_10;
        this->long_fire = _pf_script_state_checkpoint_->_pf_value_11;
        this->short_fire = _pf_script_state_checkpoint_->_pf_value_12;
...
#error "Generated lifecycle reset requires a matching PineForge engine; rebuild with script-run preparation support"
    ...
    void on_source_bar(const Bar& bar) override {
        ...
        r = (history_advances_new_bar() ? _ta_rsi_1.compute(current_bar_.close) : _ta_rsi_1.recompute(current_bar_.close));
        ...
        long_fire = (long_armed && (history_advances_new_bar() ? _ta_crossover_2.compute(r, i_pop_lo) : _ta_crossover_2.recompute(r, i_pop_lo)));
        short_fire = (short_armed && (history_advances_new_bar() ? _ta_crossunder_3.compute(r, i_pop_hi) : _ta_crossunder_3.recompute(r, i_pop_hi)));
        if (long_fire) {
            long_armed = false;
        }
        if (short_fire) {
        ...
        if ((long_fire && ([&]{ auto _pna_l = (signed_position_size()); auto _pna_r = (0); double _pfc_l = static_cast<double>(_pna_l); double _pfc_r = static_cast<double>(_pna_r); bool _pfc_eq = (_pfc_l == _pfc_r) || (std::isfinite(_pfc_l) && std::isfinite(_pfc_r) && std::fabs(_pfc_l - _pfc_r) <= 1e-10); return !is_na(_pna_l) && !is_na(_pna_r) && ((_pfc_l < _pfc_r) || _pfc_eq); }()))) {
            strategy_entry(std::string("L"), true, na<double>(), na<double>(), 1, std::string("rsi pullback long"), "", 0, -1);
        }
        if ((short_fire && ([&]{ auto _pna_l = (signed_position_size()); auto _pna_r = (0); double _pfc_l = static_cast<double>(_pna_l); double _pfc_r = static_cast<double>(_pna_r); bool _pfc_eq = (_pfc_l == _pfc_r) || (std::isfinite(_pfc_l) && std::isfinite(_pfc_r) && std::fabs(_pfc_l - _pfc_r) <= 1e-10); return !is_na(_pna_l) && !is_na(_pna_r) && ((_pfc_l > _pfc_r) || _pfc_eq); }()))) {
