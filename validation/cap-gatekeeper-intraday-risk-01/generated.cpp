...
#error "Generated lifecycle reset requires a matching PineForge engine; rebuild with script-run preparation support"
    ...
    void prepare_script_run(const Bar* bars, int n, bool allow_precalculation) override {
        ...
        this->_ta_ema_2 = decltype(this->_ta_ema_2)(31);
        ...
        this->tradeSession = std::string("");
        this->sessionTimezone = std::string("");
        this->maxDailyFills = 0;
        this->fastLength = 0;
        this->slowLength = 0;
        this->atrLength = 0;
        this->stopAtr = 0.0;
    ...
    void on_source_bar(const Bar& bar) override {
        ...
        set_pine_risk_max_intraday_filled_orders((int)(maxDailyFills));
        newDay = tf_change(prev_bar_timestamp_, current_bar_.timestamp, std::string("1D"), syminfo_.timezone, syminfo_.session);
        inSession = !(is_na(pine_time(current_bar_.timestamp, script_tf_, tradeSession, sessionTimezone, script_tf_ PF_PINE_TIME_SESSION_DAY_ARGS(syminfo_.timezone, syminfo_.session))));
        ...
        if ((([&]{ auto _pna_l = (signed_position_size()); auto _pna_r = (0); double _pfc_l = static_cast<double>(_pna_l); double _pfc_r = static_cast<double>(_pna_r); bool _pfc_eq = (_pfc_l == _pfc_r) || (std::isfinite(_pfc_l) && std::isfinite(_pfc_r) && std::fabs(_pfc_l - _pfc_r) <= 1e-10); return !is_na(_pna_l) && !is_na(_pna_r) && ((_pfc_l > _pfc_r) && !_pfc_eq); }()) && (!(inSession) || [&](){ auto _pf_bool_v = (newDay); using _pf_bool_t = std::decay_t<decltype(_pf_bool_v)>; if constexpr (std::is_same_v<_pf_bool_t, bool>) { return _pf_bool_v; } else if constexpr (std::is_floating_point_v<_pf_bool_t> || std::is_integral_v<_pf_bool_t>) { return is_na(_pf_bool_v) ? false : (_pf_bool_v != 0); } else { return static_cast<bool>(_pf_bool_v); } }()))) {
