...
public:
    ...
    struct _PFScriptState {
        ...
        decltype(GeneratedStrategy::cog) _pf_value_4;
    ...
    void restore_script_state() override {
        ...
        this->cog = _pf_script_state_checkpoint_->_pf_value_4;
...
#error "Generated lifecycle reset requires a matching PineForge engine; rebuild with script-run preparation support"
    ...
    void prepare_script_run(const Bar* bars, int n, bool allow_precalculation) override {
        ...
        this->_ta_cog_1 = decltype(this->_ta_cog_1)(10);
    ...
    void on_source_bar(const Bar& bar) override {
        cog = (history_advances_new_bar() ? _ta_cog_1.compute(current_bar_.close) : _ta_cog_1.recompute(current_bar_.close));
        sig = (history_advances_new_bar() ? _ta_ema_2.compute(cog) : _ta_ema_2.recompute(cog));
        if ((history_advances_new_bar() ? _ta_crossover_3.compute(cog, sig) : _ta_crossover_3.recompute(cog, sig))) {
            strategy_entry(std::string("L"), true, na<double>(), na<double>(), na<double>(), std::string("cog cross up"));
        }
        if ((history_advances_new_bar() ? _ta_crossunder_4.compute(cog, sig) : _ta_crossunder_4.recompute(cog, sig))) {
