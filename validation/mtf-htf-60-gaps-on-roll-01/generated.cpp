...
public:
    ...
    void on_bar(const Bar& bar) override {
        ...
        rollDown = false;
        if (hasHtfValue) {
            rollUp = (!(is_na(lastHtfClose)) && (h60Gap > lastHtfClose));
            rollDown = (!(is_na(lastHtfClose)) && (h60Gap < lastHtfClose));
            lastHtfClose = h60Gap;
        }
        if ((rollUp && (signed_position_size() == 0))) {
            strategy_entry(std::string("GAP_UP"), true, na<double>(), na<double>(), 1, std::string("gap roll up"), "", 0, -1);
        }
        if ((rollDown && (signed_position_size() > 0))) {
