...
#error "Generated lifecycle reset requires a matching PineForge engine; rebuild with script-run preparation support"
    ...
    void on_source_bar(const Bar& bar) override {
        ...
        int _for_start_0 = (0);
        int _for_end_0 = [&](){ double _pf_v = (double)(((len - 1))); return is_na(_pf_v) ? na<int>() : (int)_pf_v; }();
        int _for_step_0 = (1);
        ...
        for (int i = _for_start_0; !is_na(_for_start_0) && !is_na(_for_end_0) && !is_na(_for_step_0) && (_for_down_0 ? (i >= _for_end_0) : (i <= _for_end_0)); i += (_for_down_0 ? -_for_step_0 : _for_step_0), _for_end_0 = [&](){ double _pf_v = (double)(((len - 1))); return is_na(_pf_v) ? na<int>() : (int)_pf_v; }()) {
