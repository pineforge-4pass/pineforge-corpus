...
#error "Generated lifecycle reset requires a matching PineForge engine; rebuild with script-run preparation support"
    ...
    void prepare_script_run(const Bar* bars, int n, bool allow_precalculation) override {
        ...
        this->insideSession = decltype(this->insideSession){};
        this->tradeSession = std::string("");
        this->tradeTimezone = std::string("");
        this->fastLength = 0;
        this->slowLength = 0;
        this->atrLength = 0;
        this->stopAtr = 0.0;
    ...
    void on_source_bar(const Bar& bar) override {
        ...
        if (history_advances_new_bar()) insideSession.push(!(is_na(pine_time(current_bar_.timestamp, script_tf_, tradeSession, tradeTimezone, script_tf_ PF_PINE_TIME_SESSION_DAY_ARGS(syminfo_.timezone, syminfo_.session)))));
        else insideSession.update(!(is_na(pine_time(current_bar_.timestamp, script_tf_, tradeSession, tradeTimezone, script_tf_ PF_PINE_TIME_SESSION_DAY_ARGS(syminfo_.timezone, syminfo_.session)))));
