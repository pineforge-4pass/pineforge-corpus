...
public:
    ...
    explicit GeneratedStrategy() : _ta_vwap_anchored_bands_1(1) {
...
#error "Generated lifecycle reset requires a matching PineForge engine; rebuild with script-run preparation support"
    ...
    void prepare_script_run(const Bar* bars, int n, bool allow_precalculation) override {
        ...
        this->_ta_vwap_anchored_bands_1 = decltype(this->_ta_vwap_anchored_bands_1)(1);
    ...
    void on_source_bar(const Bar& bar) override {
        ...
        long_condition = (history_advances_new_bar() ? _ta_crossover_2.compute(current_bar_.close, upper_band) : _ta_crossover_2.recompute(current_bar_.close, upper_band));
        exit_condition = (history_advances_new_bar() ? _ta_crossunder_3.compute(current_bar_.close, vw) : _ta_crossunder_3.recompute(current_bar_.close, vw));
        ...
        if ([&](){ auto _pf_bool_v = (exit_condition); using _pf_bool_t = std::decay_t<decltype(_pf_bool_v)>; if constexpr (std::is_same_v<_pf_bool_t, bool>) { return _pf_bool_v; } else if constexpr (std::is_floating_point_v<_pf_bool_t> || std::is_integral_v<_pf_bool_t>) { return is_na(_pf_bool_v) ? false : (_pf_bool_v != 0); } else { return static_cast<bool>(_pf_bool_v); } }()) {
            strategy_close(std::string("long"), "", na<double>(), na<double>(), false, 90194313235ULL);
        }
    }
