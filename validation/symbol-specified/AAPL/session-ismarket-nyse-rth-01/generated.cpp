...
public:
    ...
    explicit GeneratedStrategy() : _ta_sma_1(9), _ta_sma_2(21), _ta_sma_4(9), _ta_sma_5(21) {
...
#error "Generated lifecycle reset requires a matching PineForge engine; rebuild with script-run preparation support"
    ...
    void prepare_script_run(const Bar* bars, int n, bool allow_precalculation) override {
        ...
        this->_ta_sma_4 = decltype(this->_ta_sma_4)(9);
        this->_ta_sma_5 = decltype(this->_ta_sma_5)(21);
    ...
    void on_source_bar(const Bar& bar) override {
        if ([&](){ auto _pf_bool_v = (_pf_session_market_(syminfo_.session, syminfo_.timezone, script_tf_, current_bar_.timestamp)); using _pf_bool_t = std::decay_t<decltype(_pf_bool_v)>; if constexpr (std::is_same_v<_pf_bool_t, bool>) { return _pf_bool_v; } else if constexpr (std::is_floating_point_v<_pf_bool_t> || std::is_integral_v<_pf_bool_t>) { return is_na(_pf_bool_v) ? false : (_pf_bool_v != 0); } else { return static_cast<bool>(_pf_bool_v); } }()) {
            if ((history_advances_new_bar() ? _ta_crossover_3.compute((history_advances_new_bar() ? _ta_sma_1.compute(current_bar_.close) : _ta_sma_1.recompute(current_bar_.close)), (history_advances_new_bar() ? _ta_sma_2.compute(current_bar_.close) : _ta_sma_2.recompute(current_bar_.close))) : _ta_crossover_3.recompute((history_advances_new_bar() ? _ta_sma_1.compute(current_bar_.close) : _ta_sma_1.recompute(current_bar_.close)), (history_advances_new_bar() ? _ta_sma_2.compute(current_bar_.close) : _ta_sma_2.recompute(current_bar_.close))))) {
                strategy_entry(std::string("Long"), true, na<double>(), na<double>(), na<double>(), "");
            }
            if ((history_advances_new_bar() ? _ta_crossunder_6.compute((history_advances_new_bar() ? _ta_sma_4.compute(current_bar_.close) : _ta_sma_4.recompute(current_bar_.close)), (history_advances_new_bar() ? _ta_sma_5.compute(current_bar_.close) : _ta_sma_5.recompute(current_bar_.close))) : _ta_crossunder_6.recompute((history_advances_new_bar() ? _ta_sma_4.compute(current_bar_.close) : _ta_sma_4.recompute(current_bar_.close)), (history_advances_new_bar() ? _ta_sma_5.compute(current_bar_.close) : _ta_sma_5.recompute(current_bar_.close))))) {
                strategy_close(std::string("Long"), "", na<double>(), na<double>(), false, 68719476759ULL);
            }
        }
    }
