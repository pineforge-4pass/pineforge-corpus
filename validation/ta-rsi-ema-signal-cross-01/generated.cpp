...
public:
    ...
    struct _PFScriptState {
        ...
        decltype(GeneratedStrategy::signalLen) _pf_value_5;
        decltype(GeneratedStrategy::rsiVal) _pf_value_6;
        decltype(GeneratedStrategy::rsiSignal) _pf_value_7;
    ...
    void restore_script_state() override {
        ...
        this->signalLen = _pf_script_state_checkpoint_->_pf_value_5;
        this->rsiVal = _pf_script_state_checkpoint_->_pf_value_6;
        this->rsiSignal = _pf_script_state_checkpoint_->_pf_value_7;
...
#error "Generated lifecycle reset requires a matching PineForge engine; rebuild with script-run preparation support"
    ...
    void on_source_bar(const Bar& bar) override {
        ...
        rsiSignal = (history_advances_new_bar() ? _ta_ema_2.compute(rsiVal) : _ta_ema_2.recompute(rsiVal));
        longCond = ((history_advances_new_bar() ? _ta_crossover_3.compute(rsiVal, rsiSignal) : _ta_crossover_3.recompute(rsiVal, rsiSignal)) && ([&]{ auto _pna_l = (rsiVal); auto _pna_r = (50); double _pfc_l = static_cast<double>(_pna_l); double _pfc_r = static_cast<double>(_pna_r); bool _pfc_eq = (_pfc_l == _pfc_r) || (std::isfinite(_pfc_l) && std::isfinite(_pfc_r) && std::fabs(_pfc_l - _pfc_r) <= 1e-10); return !is_na(_pna_l) && !is_na(_pna_r) && ((_pfc_l < _pfc_r) && !_pfc_eq); }()));
        shortCond = ((history_advances_new_bar() ? _ta_crossunder_4.compute(rsiVal, rsiSignal) : _ta_crossunder_4.recompute(rsiVal, rsiSignal)) && ([&]{ auto _pna_l = (rsiVal); auto _pna_r = (50); double _pfc_l = static_cast<double>(_pna_l); double _pfc_r = static_cast<double>(_pna_r); bool _pfc_eq = (_pfc_l == _pfc_r) || (std::isfinite(_pfc_l) && std::isfinite(_pfc_r) && std::fabs(_pfc_l - _pfc_r) <= 1e-10); return !is_na(_pna_l) && !is_na(_pna_r) && ((_pfc_l > _pfc_r) && !_pfc_eq); }()));
        if (longCond) {
