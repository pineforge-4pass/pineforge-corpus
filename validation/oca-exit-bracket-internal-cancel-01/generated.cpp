...
public:
    ...
    struct _PFScriptState {
        ...
        decltype(GeneratedStrategy::atrVal) _pf_value_4;
        decltype(GeneratedStrategy::entryCond) _pf_value_5;
        decltype(GeneratedStrategy::entry) _pf_value_6;
    ...
    void restore_script_state() override {
        ...
        this->atrVal = _pf_script_state_checkpoint_->_pf_value_4;
        this->entryCond = _pf_script_state_checkpoint_->_pf_value_5;
        this->entry = _pf_script_state_checkpoint_->_pf_value_6;
...
#error "Generated lifecycle reset requires a matching PineForge engine; rebuild with script-run preparation support"
    ...
    void on_source_bar(const Bar& bar) override {
        ...
        entryCond = (history_advances_new_bar() ? _ta_crossover_3.compute(rsiVal, 30) : _ta_crossover_3.recompute(rsiVal, 30));
        ...
        if (([&]{ auto _pna_l = (signed_position_size()); auto _pna_r = (0); double _pfc_l = static_cast<double>(_pna_l); double _pfc_r = static_cast<double>(_pna_r); bool _pfc_eq = (_pfc_l == _pfc_r) || (std::isfinite(_pfc_l) && std::isfinite(_pfc_r) && std::fabs(_pfc_l - _pfc_r) <= 1e-10); return !is_na(_pna_l) && !is_na(_pna_r) && ((_pfc_l > _pfc_r) && !_pfc_eq); }())) {
            ...
            strategy_exit(std::string("X"), std::string("L"), (entry + (atrVal * 1.5)), (entry - (atrVal * 1.5)), na<double>(), na<double>(), na<double>(), 100.0, "", 1, std::string("GRP"), na<double>(), na<double>());
        }
    }
