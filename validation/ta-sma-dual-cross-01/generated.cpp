...
public:
    ...
    struct _PFScriptState {
        ...
        decltype(GeneratedStrategy::fast_ma) _pf_value_6;
        decltype(GeneratedStrategy::slow_ma) _pf_value_7;
        decltype(GeneratedStrategy::golden_cross) _pf_value_8;
        decltype(GeneratedStrategy::death_cross) _pf_value_9;
    ...
    void restore_script_state() override {
        ...
        this->fast_ma = _pf_script_state_checkpoint_->_pf_value_6;
        this->slow_ma = _pf_script_state_checkpoint_->_pf_value_7;
        this->golden_cross = _pf_script_state_checkpoint_->_pf_value_8;
        this->death_cross = _pf_script_state_checkpoint_->_pf_value_9;
...
#error "Generated lifecycle reset requires a matching PineForge engine; rebuild with script-run preparation support"
    ...
    void prepare_script_run(const Bar* bars, int n, bool allow_precalculation) override {
        ...
        this->fast_ma = 0.0;
        this->slow_ma = 0.0;
        this->golden_cross = false;
        this->death_cross = false;
    ...
    void on_source_bar(const Bar& bar) override {
        ...
        fast_ma = (history_advances_new_bar() ? _ta_sma_1.compute(current_bar_.close) : _ta_sma_1.recompute(current_bar_.close));
        slow_ma = (history_advances_new_bar() ? _ta_sma_2.compute(current_bar_.close) : _ta_sma_2.recompute(current_bar_.close));
        golden_cross = (history_advances_new_bar() ? _ta_crossover_3.compute(fast_ma, slow_ma) : _ta_crossover_3.recompute(fast_ma, slow_ma));
        death_cross = (history_advances_new_bar() ? _ta_crossunder_4.compute(fast_ma, slow_ma) : _ta_crossunder_4.recompute(fast_ma, slow_ma));
        if ((golden_cross && ([&]{ auto _pna_l = (signed_position_size()); auto _pna_r = (0); double _pfc_l = static_cast<double>(_pna_l); double _pfc_r = static_cast<double>(_pna_r); bool _pfc_eq = (_pfc_l == _pfc_r) || (std::isfinite(_pfc_l) && std::isfinite(_pfc_r) && std::fabs(_pfc_l - _pfc_r) <= 1e-10); return !is_na(_pna_l) && !is_na(_pna_r) && ((_pfc_l < _pfc_r) || _pfc_eq); }()))) {
            if (([&]{ auto _pna_l = (signed_position_size()); auto _pna_r = (0); double _pfc_l = static_cast<double>(_pna_l); double _pfc_r = static_cast<double>(_pna_r); bool _pfc_eq = (_pfc_l == _pfc_r) || (std::isfinite(_pfc_l) && std::isfinite(_pfc_r) && std::fabs(_pfc_l - _pfc_r) <= 1e-10); return !is_na(_pna_l) && !is_na(_pna_r) && ((_pfc_l < _pfc_r) && !_pfc_eq); }())) {
                strategy_close(std::string("S"), std::string("flip flat"), na<double>(), na<double>(), false, 206158430231ULL);
            }
            strategy_entry(std::string("L"), true, na<double>(), na<double>(), na<double>(), std::string("golden cross"));
        }
        if ((death_cross && ([&]{ auto _pna_l = (signed_position_size()); auto _pna_r = (0); double _pfc_l = static_cast<double>(_pna_l); double _pfc_r = static_cast<double>(_pna_r); bool _pfc_eq = (_pfc_l == _pfc_r) || (std::isfinite(_pfc_l) && std::isfinite(_pfc_r) && std::fabs(_pfc_l - _pfc_r) <= 1e-10); return !is_na(_pna_l) && !is_na(_pna_r) && ((_pfc_l > _pfc_r) || _pfc_eq); }()))) {
            if (([&]{ auto _pna_l = (signed_position_size()); auto _pna_r = (0); double _pfc_l = static_cast<double>(_pna_l); double _pfc_r = static_cast<double>(_pna_r); bool _pfc_eq = (_pfc_l == _pfc_r) || (std::isfinite(_pfc_l) && std::isfinite(_pfc_r) && std::fabs(_pfc_l - _pfc_r) <= 1e-10); return !is_na(_pna_l) && !is_na(_pna_r) && ((_pfc_l > _pfc_r) && !_pfc_eq); }())) {
                strategy_close(std::string("L"), std::string("flip flat"), na<double>(), na<double>(), false, 227633266711ULL);
            }
