...
struct _PFCheckpointTraits<_PFUdtRecord_Bracket> {
    struct snapshot_type {
        _PFCheckpointTraits<decltype(_PFUdtRecord_Bracket::stop_mult)>::snapshot_type _pf_field_0;
        _PFCheckpointTraits<decltype(_PFUdtRecord_Bracket::limit_mult)>::snapshot_type _pf_field_1;
        _PFCheckpointTraits<decltype(_PFUdtRecord_Bracket::base_qty)>::snapshot_type _pf_field_2;
    };
    static snapshot_type take(const _PFUdtRecord_Bracket& value) {
        return snapshot_type{
            ...
            _PFCheckpointTraits<decltype(_PFUdtRecord_Bracket::base_qty)>::take(value.base_qty),
        };
    }
    static void restore(_PFUdtRecord_Bracket& value, const snapshot_type& snapshot) {
        _PFCheckpointTraits<decltype(_PFUdtRecord_Bracket::stop_mult)>::restore(value.stop_mult, snapshot._pf_field_0);
        _PFCheckpointTraits<decltype(_PFUdtRecord_Bracket::limit_mult)>::restore(value.limit_mult, snapshot._pf_field_1);
        _PFCheckpointTraits<decltype(_PFUdtRecord_Bracket::base_qty)>::restore(value.base_qty, snapshot._pf_field_2);
    }
};
...
public:
    ...
    struct _PFScriptState {
        ...
        _PFCheckpointTraits<decltype(GeneratedStrategy::br)>::snapshot_type _pf_value_7;
        _PFCheckpointTraits<decltype(GeneratedStrategy::atrVal)>::snapshot_type _pf_value_8;
    ...
    void restore_script_state() override {
        ...
        _PFCheckpointTraits<decltype(GeneratedStrategy::br)>::restore(this->br, _pf_script_state_checkpoint_->_pf_value_7);
        _PFCheckpointTraits<decltype(GeneratedStrategy::atrVal)>::restore(this->atrVal, _pf_script_state_checkpoint_->_pf_value_8);
...
#error "Generated lifecycle reset requires a matching PineForge engine; rebuild with script-run preparation support"
    ...
    std::tuple<double, double, double> _udt_Bracket_levels(Bracket self, double price, double atr, bool isLong) {
        double stopPx = ((isLong) ? ((price - (_pf_udt_Bracket.read(self).stop_mult * atr))) : ((price + (_pf_udt_Bracket.read(self).stop_mult * atr))));
        double limitPx = ((isLong) ? ((price + (_pf_udt_Bracket.read(self).limit_mult * atr))) : ((price - (_pf_udt_Bracket.read(self).limit_mult * atr))));
        return std::make_tuple(stopPx, limitPx, _pf_udt_Bracket.read(self).base_qty);
    }

    void on_source_bar(const Bar& bar) override {
        if (!_var_initialized) {
            br = _pf_udt_Bracket.create(_PFUdtRecord_Bracket{.stop_mult = 1.5, .limit_mult = 3.0, .base_qty = 1.0});
        ...
        if (((history_advances_new_bar() ? _ta_crossover_4.compute(emaFast, emaSlow) : _ta_crossover_4.recompute(emaFast, emaSlow)) && ([&]{ auto _pna_l = (signed_position_size()); auto _pna_r = (0); double _pfc_l = static_cast<double>(_pna_l); double _pfc_r = static_cast<double>(_pna_r); bool _pfc_eq = (_pfc_l == _pfc_r) || (std::isfinite(_pfc_l) && std::isfinite(_pfc_r) && std::fabs(_pfc_l - _pfc_r) <= 1e-10); return !is_na(_pna_l) && !is_na(_pna_r) && (_pfc_eq); }()))) {
            ...
            strategy_exit(std::string("LX"), std::string("L"), limitL, stopL, na<double>(), na<double>(), na<double>(), 100.0, std::string("bracket long"), na<double>(), "", na<double>(), na<double>());
        }
        if (((history_advances_new_bar() ? _ta_crossunder_5.compute(emaFast, emaSlow) : _ta_crossunder_5.recompute(emaFast, emaSlow)) && ([&]{ auto _pna_l = (signed_position_size()); auto _pna_r = (0); double _pfc_l = static_cast<double>(_pna_l); double _pfc_r = static_cast<double>(_pna_r); bool _pfc_eq = (_pfc_l == _pfc_r) || (std::isfinite(_pfc_l) && std::isfinite(_pfc_r) && std::fabs(_pfc_l - _pfc_r) <= 1e-10); return !is_na(_pna_l) && !is_na(_pna_r) && (_pfc_eq); }()))) {
            ...
            strategy_exit(std::string("SX"), std::string("S"), limitS, stopS, na<double>(), na<double>(), na<double>(), 100.0, std::string("bracket short"), na<double>(), "", na<double>(), na<double>());
        }
    }
